...
| user | 3000 | `/`, `/chat`, `/architecture`, `/sessions`, `/search`† |
| monitor | 3002 | `/`, `/health`, `/index-status`, `/compliance`†, `/mcp`† |

† 标注路由为 Phase 2+ 或 P1 计划项，当前未实现或仅占位。
...
**计划已全部确认；Batch 0~5 与遗漏项补全已交付（2026-07-06）。**

---

## 12. 当前实现状态（2026-07-06）

Batch 0~5 及遗漏项审计补全后的**实际落地**摘要。详细差距见 [遗漏项审计计划](batch0-5_遗漏审计_8fd70d2e.plan.md)（Cursor plans）。

### 12.1 批次完成度

| 批次 | 状态 | 关键交付 |
|------|------|----------|
| Batch 0~4 | ✅ 完成 | monorepo、五服务骨架、LLM factory、JWT+SSE、Docker+Nginx 全栈 |
| Batch 5 P0-A | ✅ 基础闭环 | 代码源 CRUD、Git+indexer 索引流水线、Qdrant/Neo4j 写入、索引看板 |
| Batch 5 P0-B | ✅ 基础闭环 | RAG 向量检索、template_hint SSE、/sessions、/chat |
| Batch 5 P0-C | ✅ 基础闭环 | admin 架构发布、D3 图谱、健康/合规看板、漂移处理 |

### 12.2 服务与契约（已实现）

| 模块 | 实现要点 |
|------|----------|
| **core gRPC** | `CoreService.Ping` 已注册（`Empty` → `StringValue`） |
| **core HTTP** | api → core 内部 REST（`/internal/index`、`/internal/search`、架构草稿） |
| **索引流水线** | Git clone → `lingprism-indexer parse` → Qdrant 向量 + Neo4j 图谱 + MySQL 元数据 |
| **SearchService** | Qdrant 向量检索 + MySQL 知识库 LIKE |
| **SSE** | `status` / `token` / `source` / `template_hint` / `done` / `error` |
| **MCP Tools** | `search_code`, `search_knowledge`, `get_architecture`, `ask_question` + echo |
| **MCP 审计** | 结构化 stdout 日志 + `mcp_audit_logs` 迁移 |
| **Langfuse** | `langfuse_tracer.py`（配置 `LANGFUSE_*` 时生效） |
| **graph-viz** | D3 force-directed 布局（`@lingprism/graph-viz`） |

### 12.3 前端路由（当前）

| App | 已实现路由 |
|-----|------------|
| user | `/` `/chat` `/sessions` `/architecture` `/login` |
| admin | `/repos` `/knowledge` `/architecture` `/login` |
| monitor | `/` `/health` `/index-status` `/login` |

### 12.4 Docker Compose（`--profile app`）

除 api、core、ai-worker、mcp、三前端、nginx、migrate 外，已纳入 **indexer** 服务；**ai-worker** 含 healthcheck。

### 12.5 已知限制与 Phase 2+ 待办

- api → core 仍以 **HTTP** 为主（gRPC Ping 可用，业务 RPC 未全面迁移）
- Embedding 索引使用 **确定性 hash 向量**（联调用）；生产须换真实 LLM embedding
- 私有 Git 仓库 clone 凭据注入待完善
- **OpenSearch** 全文索引、**@auth GraphQL directive**、admin `/settings/llm`（DB 热配置）未实现
- PRD P1：`/search` 代码检索、模板管理页、MCP 监控看板、SSO 等
